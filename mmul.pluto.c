...
int main(int argc, char* argv[])
{
	...
	char czas_mnozenia[30] = "czas_mnozenia_kafelowo-";
	char czas_calkowity[30] = "czas_calkowity_kafelkow-";
	char suf[5];
	int s = sizeof(double);
	...
	*/
	...
	if ((ILEKOLUMN >= 1) && (ILEWIERSZY >= 1)) {
		lbp = 0;
		ubp = floord(ILEWIERSZY - 1, 93);
#pragma omp parallel for private(lbv,ubv)
		for (t1 = lbp; t1 <= ubp; t1++) {
			for (t2 = 0; t2 <= floord(ILEWIERSZY - 1, 95); t2++) {
				for (t3 = 0; t3 <= floord(ILEKOLUMN - 1, 97); t3++) {
					for (t4 = 93 * t1; t4 <= min(ILEWIERSZY - 1, 93 * t1 + 92); t4++) {
						for (t5 = 97 * t3; t5 <= min(ILEKOLUMN - 1, 97 * t3 + 96); t5++) {
							lbv = 95 * t2;
							ubv = min(ILEWIERSZY - 1, 95 * t2 + 94);
#pragma ivdep
